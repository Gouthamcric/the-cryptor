...
def encryption(message,key):
    ...
    for i in range(16):
        ...
        temp_list=[C[i+1],D[i+1],Key[i+1],ae,ake,SBOX_res,SBOX_res_2,Permutation_res,R[i+1],L[i+1]]
        List_1.append(temp_list)
        
        print("\n\n")
    List_0=[M_bin,res_pt,L[0],R[0],str(K_bin),res_k,C[0],D[0]]
    
    
    result = inverse_permutation(R[16]+L[16])
    print("Final Answer" , result)
    print("Hexadecimal Format",hex(int(listToString(result), 2)))
    return(List_0,List_1,hex(int(listToString(result), 2)))
